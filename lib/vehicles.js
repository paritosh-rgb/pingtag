...
function publicVehicle(vehicle) {
  ...
  return {
    ...
    societyBrandName: vehicle.society_brand_name || vehicle.societyBrandName || vehicle.tags?.society_name || "",
    ...
    civicScore: Number(vehicle.civic_score || vehicle.civicScore || 0),
  };
}
...
export async function activateVehicle({ accessToken, ownerId, origin, tagCode, ...input }) {
  ...
  if (supabaseConfigured) {
    ...
    const { data: tag, error: tagError } = await supabase.from("tags").select("id, code, status, society_name").eq("code", qrToken).single();
    ...
    const { data, error } = await supabase.from("vehicles").insert({
        ...
        society_name: input.societyName || tag.society_name || null,
    ...
    return { ...publicVehicle({ ...data, qr_data_url: qrDataUrl, tags: tag }), scanUrl, qrDataUrl, phoneNumber: input.phoneNumber, address: input.address || "" };
  }
  ...
  const tag = {
    ...
    societyName: input.societyName || inventoryTag.societyName || "",
    societyBrandName: inventoryTag.societyName || "",
...
export async function findVehicleByToken(token) {
  if (supabaseConfigured) {
    ...
    await supabase.from("tag_scans").insert({ vehicle_id: data.id });
    const { data: tag } = await supabase.from("tags").select("created_at,society_name").eq("code", token).single();
    return publicVehicle({ ...data, tags: tag });
  }
  const store = await readStore();
  const tag = store.tags.find((item) => item.qrToken?.toLowerCase() === token.toLowerCase() || item.tagCode?.toLowerCase() === token.toLowerCase());
  if (!tag) return null;
  tag.scanCount = Number(tag.scanCount || 0) + 1;
  await writeStore(store);
  return { ...tag, civicScore: tag.scanCount };
}

export async function findPrintedTag(token) {
  if (supabaseConfigured) {
    const supabase = getSupabase();
    const { data } = await supabase.from("tags").select("code, status, society_name").eq("code", token.toUpperCase()).single();
    return data;
  }
...
export async function listVehicles({ accessToken, ownerId }) {
  if (supabaseConfigured) {
    ...
    const { data: tags } = await supabase.from("tags").select("id,created_at,society_name").in("id", tagIds);
    const tagById = new Map((tags || []).map((tag) => [tag.id, tag]));
    const { data: scans } = await supabase.from("tag_scans").select("vehicle_id").in("vehicle_id", data.map((vehicle) => vehicle.id));
    const scanCounts = new Map();
    (scans || []).forEach((scan) => scanCounts.set(scan.vehicle_id, (scanCounts.get(scan.vehicle_id) || 0) + 1));
    return data.map((vehicle) => publicVehicle({ ...vehicle, civic_score: scanCounts.get(vehicle.id) || 0, tags: tagById.get(vehicle.tag_id) }));
  }
  const store = await readStore();
