...
export async function GET(request) {
  ...
  if (supabaseConfigured) {
    ...
    const { data, error } = await supabase.from("alerts").select("id, vehicle_id, reason, message, created_at").eq("vehicle_id", vehicleId).order("created_at", { ascending: false }).limit(50);
    if (error) return Response.json({ error: error.message }, { status: 400 });
    return Response.json({ alerts: data.map((alert) => ({ id: alert.id, category: alert.reason, message: alert.message, createdAt: alert.created_at, delivered: true })) });
  }
