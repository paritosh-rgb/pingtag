...
export default function AuthForm() {
  ...
  return (
    <section className="auth-card panel">
      ...
      <h1>{mode === "signup" ? "Create your owner account" : "Welcome back"}</h1>
      <p className="muted">Your phone number stays private. It is only used to authenticate you and receive vehicle alerts.</p>
      <form className="form-grid" onSubmit={submit}>
        ...
        <button className="btn" disabled={busy} type="submit">{busy ? "Please wait..." : mode === "signup" ? "Sign up" : "Login"}<span>↗</span></button>
      </form>
      {status ? <div className={`status ${status.kind}`}>{status.text}</div> : null}
      <p className="form-note">Your phone is the visible identity. PingTag uses a private internal login identity and never displays it.</p>
    </section>
  );
}
